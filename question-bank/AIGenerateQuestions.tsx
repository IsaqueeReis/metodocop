...
export const AIGenerateQuestions = () => {
  ...
  const handleGenerate = async () => {
    ...
    if (!apiKey) {
      console.error('API Key não encontrada. process.env.GEMINI_API_KEY:', process.env.GEMINI_API_KEY);
      alert('Erro de configuração: Chave da API Gemini não encontrada. Certifique-se de que a chave está configurada.');
      return;
    }
...
A questão deve ser MUITO IDÊNTICA ao estilo da banca preenchida (${board}).
O gabarito deve conter um comentário detalhado de acordo com fontes reais e citando as fontes (ex: leis, jurisprudência, doutrina).
...
- justification: A justificativa detalhada da resposta correta, citando as fontes reais.`;
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      alert(`Erro ao salvar questões: ${error.message || 'Erro desconhecido'}`);
    } finally {
      setSaving(false);
    }
  };
