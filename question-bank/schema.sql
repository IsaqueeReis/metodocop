...
-- 5. Comentários dos Alunos
CREATE TABLE IF NOT EXISTS qb_comments (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    question_id UUID REFERENCES qb_questions(id) ON DELETE CASCADE,
...
-- 6. Denúncias de Questões
CREATE TABLE IF NOT EXISTS qb_reports (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    question_id UUID REFERENCES qb_questions(id) ON DELETE CASCADE,
    user_id TEXT NOT NULL,
    user_name TEXT NOT NULL,
    report_type TEXT NOT NULL, -- Erro no enunciado, erro no gabarito, etc.
...
CREATE INDEX IF NOT EXISTS idx_qb_comments_question ON qb_comments(question_id);
CREATE INDEX IF NOT EXISTS idx_qb_reports_status ON qb_reports(status);
