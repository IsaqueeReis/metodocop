...
import { Brain, Calendar, Check, Save, Sparkles, Trash2, List, FileText, ChevronRight, Activity, Zap, Scale, BarChart2, Plus, X, AlertTriangle, Edit2, Layers } from 'lucide-react';
import { Dialog, DialogType } from '../components/ui/Dialog';

import { linkBankStorage } from './LinkBankPanel';
...
const MentorshipHeader = ({ activeMode, setActiveMode, hasPlan }: any) => {
    return (
        <div className="flex flex-wrap gap-2 mb-6">
            <button
                onClick={() => setActiveMode('DASHBOARD')}
                className={`px-4 py-2 rounded-xl text-sm font-bold uppercase transition ${activeMode === 'DASHBOARD' ? 'bg-red-600 text-white' : 'bg-zinc-900 text-zinc-400 hover:bg-zinc-800'}`}
            >
                Dashboard
            </button>
            <button
                onClick={() => setActiveMode('EDITAL')}
                className={`px-4 py-2 rounded-xl text-sm font-bold uppercase transition ${activeMode === 'EDITAL' ? 'bg-red-600 text-white' : 'bg-zinc-900 text-zinc-400 hover:bg-zinc-800'}`}
            >
                Edital Ativo
            </button>
            <button
                onClick={() => setActiveMode('PLANNER')}
                className={`px-4 py-2 rounded-xl text-sm font-bold uppercase transition ${activeMode === 'PLANNER' ? 'bg-red-600 text-white' : 'bg-zinc-900 text-zinc-400 hover:bg-zinc-800'}`}
            >
                Planner IA
            </button>
            {hasPlan && (
                <button
                    onClick={() => setActiveMode('FULL_EDIT')}
                    className={`px-4 py-2 rounded-xl text-sm font-bold uppercase transition ${activeMode === 'FULL_EDIT' ? 'bg-red-600 text-white' : 'bg-zinc-900 text-zinc-400 hover:bg-zinc-800'}`}
                >
                    Gestão Completa
                </button>
            )}
            <button
                onClick={() => setActiveMode('SPECIFIC_GOALS')}
                className={`px-4 py-2 rounded-xl text-sm font-bold uppercase transition ${activeMode === 'SPECIFIC_GOALS' ? 'bg-red-600 text-white' : 'bg-zinc-900 text-zinc-400 hover:bg-zinc-800'}`}
            >
                Espec. Metas
            </button>
        </div>
...
interface AdminMentorshipPanelProps {
    ...
    plans: Plan[];
    editais: Edital[];
    onUpdateEditais?: (editais: Edital[]) => void;
    onUpdateUser?: (updatedUser: User) => void;
}

export const AdminMentorshipPanel = ({ users, plans, editais, onUpdateEditais, onUpdateUser }: AdminMentorshipPanelProps) => {
    ...
    const [activeMode, setActiveMode] = useState<'EDITAL' | 'DASHBOARD' | 'PLANNER' | 'FULL_EDIT'>('DASHBOARD');
    const [newMissage, setNewMissage] = useState('');
    ...
    const [plannerInputMode, setPlannerInputMode] = useState<'EDITAL' | 'FLEXIVEL' | 'SAVED'>('EDITAL');
    const [useLinkSequence, setUseLinkSequence] = useState(true);
    const [planStudyMode, setPlanStudyMode] = useState<'CICLOS' | 'CRONOGRAMA'>('CRONOGRAMA');
    ...
    const handleLoadFromEdital = () => {
        const student = premiumStudents.find(s => s.id === selectedStudentId);
        if (!student || !student.assignedEditalId) {
            showAlert("Aviso", "Aluno sem edital ativo. Vincule um edital primeiro.");
            return;
        }
        const edital = editais.find(e => e.id === student.assignedEditalId);
        if (!edital) return;

        const structure: DetectedSubject[] = edital.subjects.map(s => ({
            name: s.name,
    ...
    const [selectedEditalForGoals, setSelectedEditalForGoals] = useState<string>('');
    const [specificGoalsSearch, setSpecificGoalsSearch] = useState('');
    
    useEffect(() => {
        MentorshipStorage.getGlobalGoalsMetadata().then(data => setGlobalGoals(data));
    }, []);
    
    const handleSaveGlobalGoal = async (subject: string, topic: string, specificMessage: string) => {
        let updated = [...globalGoals];
        const existing = updated.findIndex(g => g.subject === subject && g.topic === topic);
        if (existing !== -1) {
            updated[existing].specificMessage = specificMessage;
        } else {
            updated.push({ subject, topic, specificMessage });
        }
        setGlobalGoals(updated);
        await MentorshipStorage.saveGlobalGoalsMetadata(updated);
    };
    ...
    // --- SISTEMA DE DIíÂLOGO CUSTOMIZADO ---
    ...
    const handleSendMissage = async () => {
        if (!currentPlan || !newMissage) return;
        const today = new Date();
        const todayStr = getLocalDateStr(today);
        const filteredMissages = currentPlan.messages.filter(m => m.date !== todayStr);
        const updatedPlan = { ...currentPlan, messages: [...filteredMissages, { id: crypto.randomUUID(), date: todayStr, content: newMissage, isRead: false }] };
        try {
            await MentorshipStorage.savePlan(updatedPlan);
        } catch (err) {
    ...
    const confirmSaveEdital = async () => {
        ...
        try {
            ...
            if (onUpdateEditais) {
                onUpdateEditais([...editais, newEdital]);
            }
    ...
    const handleGenerateSchedule = async () => {
        ...
        try {
            ...
            if (useLinkSequence) {
                const student = premiumStudents.find(s => s.id === selectedStudentId);
                if (student && student.assignedEditalId) {
                    const sequences = await linkBankStorage.getSequences();
                    currentSequence = sequences.find((s) => s.editalId === student.assignedEditalId);
                }
            }

            if (currentSequence && currentSequence.items && currentSequence.items.length > 0) {
                // USAR A SEQUÊNCIA DE LINKS (Ignorar IA)
                studyQueue = currentSequence.items.map(item => ({
                    subject: item.subjectName,
                    topic: item.topicName,
                    instructions: "Estudo teórico e prático (conforme material)"
                }));
            } else {
                // LÓGICA LOCAL (Substituindo a IA para ser instantâneo)
                const topicsBySubject: Record<string, string[]> = {};
                const subjectNames = detectedStructure.map(s => {
                    topicsBySubject[s.name] = [...s.topics];
                    return s.name;
                });
                ...
                while (hasMore) {
                    hasMore = false;
                    // Itera sobre as matérias e pega 1 tópico de cada (intercalando)
                    for (const subjectName of subjectNames) {
                        if (topicsBySubject[subjectName].length > 0) {
                            const topic = topicsBySubject[subjectName].shift();
                            studyQueue.push({
                                subject: subjectName,
                                topic: topic,
                                instructions: "Estudo teórico e prático (foco no conteúdo)"
                            });
                            hasMore = true;
                        }
                    }
            ...
            // Se for Modo Ciclo, a distribuição não é por dia da semana, é linear e contínua.
            if (planStudyMode === 'CICLOS') {
                const finalSchedule: AIPlanItem[] = studyQueue.map((item, index) => ({
                    dayOffset: index, // Apenas para ordenação
                    subject: item.subject,
                    topic: item.topic,
                    type: "AULA",
                    instructions: item.instructions || "Estudo contínuo"
                }));

                setGeneratedItems(finalSchedule);
                setCalculatedEndDate(new Date().toLocaleDateString('pt-BR'));
                setPlannerStep('PREVIEW');
                setIsGenerating(false);
                return;
            }

            // --- ALGORITMO DE DISTRIBUIÇíO TEMPORAL (LOCAL) ---
            ...
            console.error(err);
            showAlert("Erro na Operação Estratégica", err.message || 'Falha desconhecida ao gerar cronograma.');
        } finally {
            setIsGenerating(false);
        }
    };
    ...
    const handleResetPlan = async () => {
        ...
        showConfirm("Reiniciar Ciclo", "ATENÇíO: Isso irá apagar todo o progresso do aluno e reiniciar o cronograma a partir de HOJE. O cronograma será recalculado com base nos dias de estudo definidos. Deseja continuar?", async () => {
            try {
    ...
    const handleWipePlan = async () => {
        ...
        showConfirm("Zerar Plano de Estudos", "ATENÇíO: Isso EXCLUIRíÂ PERMANENTEMENTE todas as tarefas e mensagens do plano. O aluno ficará sem cronograma. Confirmar destruição total?", async () => {
            try {
                const updatedPlan = {
                    ...currentPlan,
                    tasks: [],
                ...
                try {
                    await MentorshipStorage.savePlan(updatedPlan);
                    setCurrentPlan(updatedPlan);
                    showAlert("Plano Zerado", "Todas as tarefas foram excluídas. O campo de batalha está limpo.");
                } catch (error) {
    ...
    const confirmDeployPlan = async () => {
        ...
        try {
            // Busca sequências de links se habilitado
            let currentSequence = null;
            if (useLinkSequence) {
                const student = premiumStudents.find(s => s.id === selectedStudentId);
                if (student && student.assignedEditalId) {
            ...
            const aiTasks = generatedItems.map(item => {
                // Extract links if sequence is active
                let resumoUrl, questoesUrl, aulaUrl;
                if (currentSequence) {
                    const topicLinks = currentSequence.topics?.find((t) => t.subject === item.subject && t.topic === item.topic);
                    if (topicLinks) {
                        resumoUrl = topicLinks.resumoLink;
                        questoesUrl = topicLinks.questoesLink;
                        aulaUrl = topicLinks.aulaLink;
                    }
                }
    ...
    if (premiumStudents.length === 0) return <div className="bg-[#050505] text-zinc-500 p-8 min-h-[50vh] border border-zinc-900 rounded-xl flex items-center justify-center"><p>Nenhum aluno cadastrado na plataforma para gerenciar mentoria.</p></div>;

    return (
        <div className="bg-[#050505] text-white p-6 min-h-screen font-sans rounded-xl relative overflow-hidden">
            {/* Background Dinâmico Tecnológico */}
            ...
            <div className="relative z-10 space-y-6">
                <header className="mb-8 bg-zinc-900/50 backdrop-blur-xl border border-white/5 p-6 rounded-2xl shadow-[0_0_20px_rgba(0,0,0,0.5)] flex flex-col md:flex-row justify-between items-center gap-4 transition-all hover:border-white/10">
                    <div><h1 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white via-zinc-400 to-zinc-700 uppercase tracking-tight" style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 800 }}>Comando de <span className="text-red-600">Mentoria</span></h1><p className="text-zinc-500 text-sm font-bold">Painel de Controle Individual</p></div>
                    <div className="flex items-center gap-4">
                        <div className="flex bg-black/40 backdrop-blur-md p-1 rounded-xl border border-white/5 shadow-inner">
                            <button onClick={() => setActiveMode('EDITAL')} className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wide flex items-center gap-2 transition-all ${activeMode === 'EDITAL' ? 'bg-zinc-800 text-white shadow' : 'text-zinc-500 hover:text-zinc-300'}`}><FileText size={14} /> Edital Ativo</button>
                            <button onClick={() => setActiveMode('DASHBOARD')} className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wide flex items-center gap-2 transition-all ${activeMode === 'DASHBOARD' ? 'bg-zinc-800 text-white shadow' : 'text-zinc-500 hover:text-zinc-300'}`}><List size={14} /> Resumo do Aluno</button>
                            <button onClick={() => setActiveMode('PLANNER')} className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wide flex items-center gap-2 transition-all ${activeMode === 'PLANNER' ? 'bg-red-900/30 text-red-400 border border-red-900/50' : 'text-zinc-500 hover:text-zinc-300'}`}><Brain size={14} /> Criador de Planos</button>
                            <button onClick={() => setActiveMode('FULL_EDIT')} className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wide flex items-center gap-2 transition-all ${activeMode === 'FULL_EDIT' ? 'bg-zinc-800 text-white shadow' : 'text-zinc-500 hover:text-zinc-300'}`}><Edit2 size={14} /> Gestão Completa</button>
                                            <button onClick={() => setActiveMode('SPECIFIC_GOALS')} className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wide flex items-center gap-2 transition-all ${activeMode === 'SPECIFIC_GOALS' ? 'bg-zinc-800 text-white shadow' : 'text-zinc-500 hover:text-zinc-300'}`}><Sparkles size={14} /> Espec. Metas</button>
                        </div>
                        <div className="flex items-center gap-2 bg-black/40 backdrop-blur-md p-2 rounded-xl border border-white/5"><span className="text-xs font-bold text-zinc-400 uppercase ml-2">Operador:</span><select value={selectedStudentId} onChange={(e) => setSelectedStudentId(e.target.value)} className="bg-zinc-900/80 text-white text-sm p-2 rounded-lg border border-white/10 outline-none focus:border-red-600 transition-all font-bold">{premiumStudents.map(s => (<option key={s.id} value={s.id}>{s.name} {s.subscriptionPlan === 'METODO_COP' ? '👑 (MÉTODO COP)' : ''}</option>))}</select></div>
                    </div>
                </header>

                {loading && <div className="text-zinc-500 text-center py-12">Carregando dados acadêmicos do aluno...</div>}

                {/* --- MODO: EDITAL ATIVO --- */}
                {!loading && activeMode === 'EDITAL' && (
                    <div className="animate-fade-in bg-black/40 backdrop-blur-xl p-8 rounded-2xl border border-white/5 shadow-[0_0_20px_rgba(0,0,0,0.5)] max-w-3xl mx-auto mt-8">
                        <div className="text-center mb-8">
                            <div className="w-16 h-16 bg-red-900/20 text-red-500 rounded-full flex items-center justify-center mx-auto mb-4">
                                <FileText size={32} />
                            </div>
                            <h2 className="text-2xl font-bold text-white mb-2">Edital Ativo Obrigatório</h2>
                            <p className="text-zinc-400">
                                Este aluno ainda não possui um Edital Ativo vinculado.
                                Para iniciar a Mentoria, você precisa selecionar o concurso para o qual o aluno está se preparando.
                                Isso configurará toda a plataforma (Trilha, Simulados, etc).
                            </p>
                        </div>
                        <div className="space-y-4">
                            <label className="block text-sm font-bold text-zinc-400 uppercase tracking-wider">Selecione o Edital Verticalizado</label>
                            <select
                                value={premiumStudents.find(s => s.id === selectedStudentId)?.assignedEditalId || ''}
                                onChange={(e) => {
                                    const student = premiumStudents.find(s => s.id === selectedStudentId);
                                    if (student) {
                                        const updated = { ...student, assignedEditalId: e.target.value };
                                        onUpdateUser(updated);
                                        setActiveMode('DASHBOARD');
                                        showAlert("Edital Vinculado", "Edital ativo atualizado com sucesso. Toda a plataforma do aluno agora reflete este edital.");
                                    }
                                }}
                                className="w-full bg-zinc-950 text-white border border-zinc-800 p-4 rounded-xl outline-none focus:border-red-600 transition"
                            >
                                <option value="">Nenhum edital selecionado</option>
                                {editais.map(e => <option key={e.id} value={e.id}>{e.title}</option>)}
                            </select>
                            {editais.length === 0 && (
                                <p className="text-red-400 text-sm mt-2 font-medium">Nenhum edital cadastrado no sistema. Vá até a aba "Gestão Editais" e crie um primeiro.</p>
                            )}
                        </div>
                    </div>
                )}
                ...
                {!loading && currentPlan && activeMode === 'PLANNER' && (
                    <div className="animate-fade-in">
                        {/* STEP 1: INPUT */}
                        {plannerStep === 'INPUT' && (
                            <div className="bg-black/40 backdrop-blur-xl p-8 rounded-2xl border border-white/5 shadow-[0_0_20px_rgba(0,0,0,0.5)] max-w-3xl mx-auto">
                                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                                    ...
                                    <div className="flex bg-zinc-950 p-1 rounded-lg border border-zinc-800">
                                        <button
                                            onClick={() => setPlannerInputMode('EDITAL')}
                                            className={`px-3 py-1.5 rounded text-[10px] font-bold uppercase transition ${plannerInputMode === 'EDITAL' ? 'bg-zinc-800 text-white shadow' : 'text-zinc-500 hover:text-zinc-300'}`}
                                        >
                                            Edital Ativo
                                        </button>
                                        <button
                                            onClick={() => setPlannerInputMode('FLEXIVEL')}
                                            className={`px-3 py-1.5 rounded text-[10px] font-bold uppercase transition ${plannerInputMode === 'FLEXIVEL' ? 'bg-zinc-800 text-white shadow' : 'text-zinc-500 hover:text-zinc-300'}`}
                                        >
                                            Plano Flexível
                                        </button>
                                        <button
                                ...
                                {plannerInputMode === 'EDITAL' ? (
                                    <div className="animate-fade-in text-center py-8">
                                        <FileText size={48} className="mx-auto text-blue-500 mb-4" />
                                        <h4 className="text-xl font-bold text-white mb-2">Baseado no Edital Ativo</h4>
                                        <p className="text-zinc-400 text-sm mb-6 max-w-md mx-auto">
                                            O sistema usará a estrutura exata do Edital Verticalizado que você vinculou a este aluno.
                                            Não é necessário colar textos.
                                        </p>
                                        <button
                                            onClick={handleLoadFromEdital}
                                            className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-lg inline-flex items-center justify-center gap-2 transition"
                                        >
                                            <Layers size={18} /> Carregar Estrutura do Edital
                                        </button>
                                    </div>
                                ) : plannerInputMode === 'FLEXIVEL' ? (
                                    <div className="animate-fade-in">
                                        <p className="text-zinc-400 text-xs mb-4">Cole abaixo as matérias/assuntos. A IA irá estruturar o plano flexível.</p>
                                        <textarea
                                            value={rawEdital}
                                            onChange={e => setRawEdital(e.target.value)}
                                            placeholder="Ex: Português (Crase, Pontuação)\nMatemática (Frações, Porcentagem)..."
                                            className="w-full bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 rounded-lg p-4 text-white h-64 outline-none focus:border-red-600 font-mono text-xs mb-4"
                                        />
                                        <button
                                        ...
                                        >
                                            ...
                                            {isAnalyzing ? "Processando Inteligência..." : "Estruturar Plano Flexível"}
                                        </button>
                                    </div>
                                ) : (
                                                ...
                                                <div key={plan.id} className="flex items-center justify-between bg-zinc-950 p-4 rounded-xl border border-zinc-800 hover:border-zinc-700 transition-colors group">
                                                    <div>
                                                        ...
                                                        <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-wider mt-1">
                                                            {plan.createdAt ? new Date(plan.createdAt).toLocaleDateString() : 'Data desconhecida'} ââ‚¬Â¢ {plan.items?.length || 0} matérias
                                                        </p>
                                                    </div>
                        ...
                        {plannerStep === 'CONFIG' && (
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                ...
                                <div className="space-y-6">
                                    <div className="bg-zinc-900/80 backdrop-blur-md border border-white/10 p-6 rounded-xl">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="space-y-3 mt-4 pt-4 border-t border-zinc-800">
                                                <div className="flex items-center justify-between">
                                                    <span className="text-sm text-white">Modo de Estudo</span>
                                                    <select
                                                        value={planStudyMode}
                                                        onChange={e => setPlanStudyMode(e.target.value as 'CICLOS' | 'CRONOGRAMA')}
                                                        className="bg-zinc-950 border border-zinc-700 rounded p-1 text-white text-xs"
                                                    >
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <input type="checkbox" checked={useLinkSequence} onChange={e => setUseLinkSequence(e.target.checked)} className="w-4 h-4 accent-blue-600" />
                                                    <span className="text-sm text-white">Utilizar Banco de Links (Preenchimento Automático)</span>
                                                </div>
                                            </div>
                                        </div>
                                        <button
                                        ...
                                        >
                                            {isGenerating ? (
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span>Calculando Rota (pode demorar ~45s)...</span>
                                                </div>
                                            ) : (
                        ...
                        {plannerStep === 'PREVIEW' && (
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                ...
                                <div className="lg:col-span-2 bg-zinc-900/50 p-6 rounded-xl border border-zinc-800 flex flex-col h-[600px]">
                                    <div className="flex justify-between items-center mb-4">
                                        <div>
                                            ...
                                            {/* DATA DE Tí‰RMINO CALCULADA - DESTAQUE MíÂXIMO */}
                                        ...
                                        <span className="bg-green-900/20 text-green-500 text-xs font-bold px-2 py-1 rounded border border-green-900">{generatedItems.length} Tarefas</span>
                                    </div>
                                    <div className="flex-1 bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 rounded-lg overflow-y-auto p-4 custom-scrollbar space-y-2">
                                ...
                                {/* SIDEBAR: METAS EXTRAS & CONFIRMAÇíO */}
                                <div className="space-y-6">
                                    <div className="bg-zinc-900/80 backdrop-blur-md border border-white/10 p-6 rounded-xl">
                                        <div className="flex justify-between items-center mb-4">
                                        ...
                                        {showExtraGoalForm && (
                                            <div className="bg-zinc-950 p-3 rounded border border-zinc-700 mb-4 animate-fade-in">
                                                <input value={newExtraGoal.title} onChange={e => setNewExtraGoal({ ...newExtraGoal, title: e.target.value })} placeholder="Título (ex: TAF)" className="w-full bg-zinc-900/80 backdrop-blur-md border border-white/10 rounded p-2 text-xs text-white mb-2" />
                                                <select value={newExtraGoal.type} onChange={e => setNewExtraGoal({ ...newExtraGoal, type: e.target.value as TaskType })} className="w-full bg-zinc-900/80 backdrop-blur-md border border-white/10 rounded p-2 text-xs text-white mb-2">
                        ...
                        <div className="bg-zinc-900/80 backdrop-blur-md border border-white/10 rounded-2xl p-6">
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                                <div>
                                    ...
                                    <p className="text-zinc-500 text-sm">Altere matérias, datas e tipos de tarefa manualmente.</p>
                                </div>
                                <button
                                ...
                                >
                                    <Save size={18} /> SALVAR ALTERAÇí•ES
                                </button>
                            </div>
                                            ...
                                            <tr key={task.id} className="hover:bg-zinc-800/30 transition-colors group">
                                                <td className="py-2 px-4">
                                                    <input
                                                        ...
                                                        className="bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 text-white text-xs p-1.5 rounded focus:border-red-600 outline-none"
                                                    />
                                                </td>
                                                ...
                                                <td className="py-2 px-4">
                                                    <input
                                                        ...
                                                        className="bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 text-white text-xs p-1.5 rounded w-full focus:border-red-600 outline-none"
                                                    />
                                                </td>
                                                <td className="py-2 px-4">
                                                    <select
                                                        ...
                                                        className="bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 text-white text-xs p-1.5 rounded focus:border-red-600 outline-none w-full"
                                                    >
                                                        {TASK_TYPES.map(tt => (
                            ...
                            {currentPlan.tasks.length === 0 && (
                                <div className="py-20 text-center text-zinc-600 italic">
                                    Nenhuma tarefa encontrada para este plano.
                                </div>
                            )}
                        </div>
                    </div>
                ...
                {/* --- MODO: DASHBOARD (RESUMO DO ALUNO) --- */}
                
            {activeMode === 'SPECIFIC_GOALS' && (
                <div className="bg-zinc-900 rounded-2xl p-6 border border-zinc-800">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-xl font-bold text-white flex items-center gap-2"><Sparkles className="text-red-500" size={24} /> Especificidade em Metas (Global)</h2>
                    </div>
                    
                    <div className="mb-6 space-y-4">
                        <div>
                            <label className="text-sm font-bold text-zinc-400 mb-2 block">Selecione o Edital Base</label>
                            <select 
                                value={selectedEditalForGoals}
                                onChange={e => setSelectedEditalForGoals(e.target.value)}
                                className="w-full bg-black border border-zinc-800 rounded-xl px-4 py-3 text-white focus:border-red-500 focus:ring-1 focus:ring-red-500 outline-none transition-all"
                            >
                        ...
                        {selectedEditalForGoals && (
                            <div>
                                <label className="text-sm font-bold text-zinc-400 mb-2 block">Buscar Tópico ou Matéria</label>
                                <input
                                    type="text"
                                    placeholder="Ex: Constitucional, Direitos Fundamentais..."
                                    value={specificGoalsSearch}
                                    onChange={e => setSpecificGoalsSearch(e.target.value)}
                    ...
                    {selectedEditalForGoals && (
                        <div className="space-y-8">
                            {editais.find(e => e.id === selectedEditalForGoals)?.subjects
                                .filter(sub => 
                                    !specificGoalsSearch || 
                                    sub.name.toLowerCase().includes(specificGoalsSearch.toLowerCase()) ||
                                    sub.topics.some(t => t.name.toLowerCase().includes(specificGoalsSearch.toLowerCase()))
                                )
                                .map(subject => (
                                <div key={subject.id} className="bg-black/50 p-6 rounded-2xl border border-zinc-800">
                                    <h3 className="text-lg font-bold text-red-500 mb-4">{subject.name}</h3>
                                    <div className="space-y-4">
                                        {subject.topics
                                            .filter(t => !specificGoalsSearch || t.name.toLowerCase().includes(specificGoalsSearch.toLowerCase()) || subject.name.toLowerCase().includes(specificGoalsSearch.toLowerCase()))
                                            .map(topic => {
                                            const existingMsg = globalGoals.find(g => g.subject === subject.name && g.topic === topic.name)?.specificMessage || '';
                                            return (
                                                <div key={topic.id} className="flex flex-col md:flex-row gap-4 items-start md:items-center bg-zinc-900/50 p-4 rounded-xl">
                                                    <div className="flex-1">
                                                        <span className="text-white font-medium block">{topic.name}</span>
                                                    </div>
                                                    <div className="w-full md:w-2/3">
                                                        <input 
                                                            type="text"
                                                            placeholder="Mensagem Específica de Meta (opcional)..."
                                                            key={existingMsg || topic.id}
                                                            defaultValue={existingMsg}
                                                            onBlur={(e) => {
                                                                if(e.target.value !== existingMsg) {
                                                                    handleSaveGlobalGoal(subject.name, topic.name, e.target.value);
                                                                }
                                                            }}
                                                            className="w-full bg-black border border-zinc-700 rounded-lg px-4 py-2 text-sm text-white focus:border-red-500 outline-none transition-all placeholder:text-zinc-600"
                                                        />
                                                    </div>
                                                </div>
                                            );
            ...
            {!loading && currentPlan && activeMode === 'DASHBOARD' && (
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade-in">
                        ...
                        <div className="space-y-8">
                            <div className="bg-zinc-900/50 p-6 rounded-xl border border-zinc-800">
                                ...
                                <textarea value={newMissage} onChange={(e) => setNewMissage(e.target.value)} placeholder="Escreva a orientação estratégica para hoje..." className="w-full bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 rounded-lg p-3 text-sm text-white h-32 outline-none focus:border-red-600 mb-4 resize-none" />
                                <button onClick={handleSendMissage} className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-2 rounded-lg flex items-center justify-center gap-2 transition-all shadow-lg shadow-red-900/20"><Icon name="send" /> Transmitir Ordem</button>
                            </div>
                            <div className="bg-zinc-900/50 p-6 rounded-xl border border-zinc-800">
                                <h3 className="text-white font-bold mb-4 uppercase text-sm tracking-wide">Adicionar Tarefa Manualmente</h3>
                                <div className="space-y-3">
                                    <div><label className="text-xs text-zinc-500 font-bold uppercase">Dia da Semana</label><select value={taskDay} onChange={e => setTaskDay(e.target.value)} className="w-full bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 rounded p-2 text-sm text-white mt-1">{DAYS_OF_WEEK.map(d => <option key={d} value={d}>{d}</option>)}</select></div>
                                    <div><label className="text-xs text-zinc-500 font-bold uppercase">Tipo de Tarefa</label><select value={taskType} onChange={e => setTaskType(e.target.value as TaskType)} className="w-full bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 rounded p-2 text-sm text-white mt-1">{TASK_TYPES.map(t => <option key={t.type} value={t.type}>{t.label}</option>)}</select></div>
                                    <div><label className="text-xs text-zinc-500 font-bold uppercase">Matéria / Tópico</label><input type="text" value={taskSubject} onChange={e => setTaskSubject(e.target.value)} placeholder="Ex: Direito Penal - Art. 121" className="w-full bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 rounded p-2 text-sm text-white mt-1" /></div>
                                    <div><label className="text-xs text-zinc-500 font-bold uppercase">Detalhes (Opcional)</label><input type="text" value={taskDesc} onChange={e => setTaskDesc(e.target.value)} placeholder="Ex: Ler PDF pág. 10 a 30" className="w-full bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 rounded p-2 text-sm text-white mt-1" /></div>
                                    <div><label className="text-xs text-zinc-500 font-bold uppercase">Repetir por (Semanas)</label><input type="number" min="1" max="52" value={taskWeeks} onChange={e => setTaskWeeks(parseInt(e.target.value) || 1)} className="w-full bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 rounded p-2 text-sm text-white mt-1" /></div>
                            ...
                            <div className="bg-zinc-900/50 p-6 rounded-xl border border-zinc-800">
                                <h3 className="text-white font-bold mb-4 uppercase text-sm tracking-wide">Meta Específica (Data íšnica)</h3>
                                <div className="space-y-3">
                                    <div><label className="text-xs text-zinc-500 font-bold uppercase">Data</label><input type="date" value={specificDate} onChange={e => setSpecificDate(e.target.value)} className="w-full bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 rounded p-2 text-sm text-white mt-1" /></div>
                                    <div><label className="text-xs text-zinc-500 font-bold uppercase">Tipo</label><select value={specificType} onChange={e => setSpecificType(e.target.value as TaskType)} className="w-full bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 rounded p-2 text-sm text-white mt-1">{TASK_TYPES.map(t => <option key={t.type} value={t.type}>{t.label}</option>)}</select></div>
                                    <div><label className="text-xs text-zinc-500 font-bold uppercase">Matéria / Tópico</label><input type="text" value={specificSubject} onChange={e => setSpecificSubject(e.target.value)} placeholder="Ex: Direito Penal - Art. 121" className="w-full bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 rounded p-2 text-sm text-white mt-1" /></div>
                                    <div><label className="text-xs text-zinc-500 font-bold uppercase">Detalhes (Opcional)</label><input type="text" value={specificDesc} onChange={e => setSpecificDesc(e.target.value)} placeholder="Ex: Ler PDF pág. 10 a 30" className="w-full bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 rounded p-2 text-sm text-white mt-1" /></div>
                                    <button onClick={handleAddSpecificTask} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 rounded-lg flex items-center justify-center gap-2 mt-2 transition-all"><Icon name="plus" /> Adicionar Meta íšnica</button>
                                </div>
                            </div>
                ...
                {editingTask && (
                    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                        <div className="bg-zinc-900/80 backdrop-blur-md border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-2xl">
                            <h3 className="text-xl font-bold text-white mb-4 uppercase tracking-tight">Editar Tarefa</h3>
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 rounded-lg p-3 text-white focus:outline-none focus:border-red-600 transition-colors"
                                    >
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        className="w-full bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 rounded-lg p-3 text-white focus:outline-none focus:border-red-600 transition-colors min-h-[100px]"
                                    />
                                </div>
                            </div>
                ...
                {showSavePlanModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fade-in">
                        <div className="bg-zinc-900/80 backdrop-blur-md border border-white/10 rounded-2xl p-8 max-w-md w-full shadow-2xl relative">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 rounded p-3 text-white outline-none focus:border-green-600"
                ...
                {editingSubjectIndex !== null && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fade-in">
                        <div className="bg-zinc-900/80 backdrop-blur-md border border-white/10 rounded-2xl p-8 max-w-2xl w-full shadow-2xl relative">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 rounded p-3 text-white outline-none focus:border-blue-600"
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        className="w-full bg-black/40 backdrop-blur-md border border-white/5 hover:border-white/20 rounded p-3 text-white outline-none focus:border-blue-600 min-h-[200px] custom-scrollbar"
                        ...
                        <div className="bg-zinc-900/80 backdrop-blur-md border border-white/10 rounded-2xl p-8 max-w-md w-full shadow-2xl relative">
                            ...
                            <div className="text-center mb-6 mt-4">
                                ...
                                <p className="text-zinc-300 text-sm leading-relaxed">
                                    Você está prestes a adicionar <span className="text-white font-bold">{generatedItems.length} novas tarefas</span> ao calendário do aluno.
                                    <br /><br />
                            ...
                            <div className="space-y-3">
                                ...
                                >
                                    <Check size={20} /> CONFIRMAR IMPLANTAÇíO
                                </button>
